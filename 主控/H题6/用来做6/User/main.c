...
/* ========================= 电机参数 =========================
 * Emm固件，1.8度步进电机，256细分。所有角度均为相对零点的绝对角度。 */
...
/* ========================= 控制参数 =========================
 * K230坐标：球左正、球右负；电机CW降低摆杆右端。 */
#define CENTER_TARGET_X           100.0f
#define KP                       0.05f
#define KI                       0.045f
#define KD                       0.013f
...
#define RECOVERY_ANGLE_DEG       7.0f
...
static void OLED_ShowStatus(int16_t x, uint8_t valid)
{
    ...
    OLED_ShowSignedNum(1, 11, (int32_t)CENTER_TARGET_X, 4);
...
static void Control_Update(int16_t x, float dt)
{
    ...
    position_error = (float)x - CENTER_TARGET_X;

    if (!filter_ready)
    {
        Control_Reset(position_error);
        filter_ready = 1;
        return;
    }

    filtered_x += X_FILTER * (position_error - filtered_x);
...
/* 收到有效坐标后退出找回状态，并恢复正常位置PID。 */
static void ProcessValidVision(const VisionSample *vision,
                               uint32_t dt_ms,
                               RecoveryState *recovery)
{
    ...
    if (recovery->active)
    {
        ...
        Control_Reset((float)vision->x - CENTER_TARGET_X);
        recovery->active = 0;
    }

    Control_Update(vision->x, (float)dt_ms * 0.001f);
    recovery->last_valid_x = vision->x;
    recovery->last_valid_ms = ms;
    recovery->have_valid_position = 1;
}

/* K230整条串口停止时无法判断球的位置，因此只执行安全回零。 */
static uint8_t HandleVisionTimeout(uint32_t last_vision_ms,
                                   RecoveryState *recovery)
{
...
/* K230仍在发包但valid=0时，在正负找回角度之间往复扫描。 */
static void HandleLostBall(RecoveryState *recovery)
{
    if (!recovery->active && !recovery->attempted &&
        (uint32_t)(ms - recovery->last_valid_ms) >= LOST_CONFIRM_MS)
    {
        /* 有历史位置时先向中心方向推；未知位置时从正方向开始。 */
        recovery->angle = (recovery->have_valid_position &&
                           recovery->last_valid_x < CENTER_TARGET_X) ?
                          -RECOVERY_ANGLE_DEG : RECOVERY_ANGLE_DEG;
        Motor_MoveAbsolute(recovery->angle);
        recovery->active = 1;
        recovery->attempted = 1;
        recovery->start_ms = ms;
        recovery->sweep_ms = ms;
    }

    if (recovery->active &&
        (uint32_t)(ms - recovery->start_ms) >= RECOVERY_TIMEOUT_MS)
    {
        Motor_GoZero();
        Control_Reset(0.0f);
        recovery->active = 0;
        recovery->have_valid_position = 0;
    }
    else if (recovery->active &&
             (uint32_t)(ms - recovery->sweep_ms) >= RECOVERY_SWEEP_MS)
    {
        recovery->angle = -recovery->angle;
        Motor_MoveAbsolute(recovery->angle);
        recovery->sweep_ms = ms;
    }
}
...
int main(void)
{
    ...
    recovery.angle = RECOVERY_ANGLE_DEG;
    ...
    while (1)
    {
        ...
        if (vision.sequence != last_sequence)
        {
            ...
            if (vision.valid && dt_ms >= 5U && dt_ms <= 100U)
                ProcessValidVision(&vision, dt_ms, &recovery);
        }
        ...
        if (HandleVisionTimeout(last_vision_ms, &recovery))
            continue;

        if (!vision.valid)
            HandleLostBall(&recovery);
    }
}
