...
/**
 * @note    引脚：PA26 (A相), PA27 (B相) — 对应 SysConfig Encoders_E1/E2
...
/**
 * @brief  获取编码器4增量值（对齐参考工程命名，返回int16_t）
 * @retval 增量值（读后清零）
 */
int16_t Encoder4_Get(void)
{
    return (int16_t)Encoder_GPIO_Get();
}

/**
 * @brief  GPIO 中断服务函数
 *         E1 (PA26/A相) 上升沿：读 B 相判断方向
 *         E2 (PA27/B相) 上升沿：读 A 相判断方向
 */
