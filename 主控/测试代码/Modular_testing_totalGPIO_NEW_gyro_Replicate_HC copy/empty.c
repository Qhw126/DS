...
static uint8_t frame_incomplete = 0;  /* 标记是否有不完整帧 */
...
int main(void)
{
    ...
    OLED_ShowString(0 * CHAR_W, LINE4_Y, (u8 *)"BLE:", FONT_SIZE);
    OLED_Refresh();
    ...
    while (1)
    {
        ...
        if (BLERX_FLAG == 1)
        {
            ...
            OLED_ShowString(0 * CHAR_W, LINE4_Y, (u8 *)"BLE:", FONT_SIZE);
            OLED_ShowString(4 * CHAR_W, LINE4_Y, (u8 *)last_ble_data, FONT_SIZE);

            /* 完整帧，回复 <OK> */
            BLE_send_String((unsigned char *)"<OK>");
        ...
        {
            OLED_ClearArea(0, LINE4_Y, 127, 64);
            OLED_ShowString(0 * CHAR_W, LINE4_Y, (u8 *)"BLE:ERR", FONT_SIZE);
            BLE_send_String((unsigned char *)"<NO>");
            OLED_Refresh();
            frame_incomplete = 0;
        }
