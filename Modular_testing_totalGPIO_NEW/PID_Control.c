...
/* 启动延时阈值（50次×20ms = 1秒，上电后等电机稳定再启用PID） */
...
void PID_Control_Init(void)
{
    ...
    PID_MotorA.Kd = 0.0f;
    PID_MotorA.Target_Iint = 70.0f;     /* 初始目标速度 */
    ...
    /* 预计算 totalRpm 限幅值：MOTOR_RATED_PWM / FEEDFORWARD_GAIN ≈ 400 RPM */
    maxRpm = MOTOR_RATED_PWM / FEEDFORWARD_GAIN;
}
...
{
    /* 启动延时：等待 STARTUP_DELAY 个周期后再启用PID */
    ...
    int32_t pulseB = Encoder_GPIO_Get();
     ...
     *
     * totalRpm 限幅说明：
     *   Target(70) + Out(±200) = ±270 RPM
     *   × FEEDFORWARD_GAIN(1.543) = ±417 PWM < MOTOR_RATED_PWM(617)
     *   余量 = 617 - 417 = 200 PWM
     *   若 Target 运行中被修改，totalRpm 需限幅防止超 MOTOR_RATED_PWM
     */
    ...
    /* totalRpm 限幅：保证 ×FEEDFORWARD_GAIN 后不超 MOTOR_RATED_PWM */
    if (totalRpmA > maxRpm) totalRpmA = maxRpm;
    if (totalRpmA < -maxRpm) totalRpmA = -maxRpm;
    ...
    if (totalRpmB > maxRpm) totalRpmB = maxRpm;
    if (totalRpmB < -maxRpm) totalRpmB = -maxRpm;
