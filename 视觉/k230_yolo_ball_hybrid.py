...
def pixel_x_to_centered_x(pixel_x):
...
def image_x(display_coordinate_x):
    return int(
...
def send_ball_packet(uart, valid, x, y):
...
def select_ball(detections, last_center):
    ...
    try:
        ...
        while True:
            ...
            pipeline.osd_img.draw_string_advanced(
                8, 8, 22, status, color=(255, 255, 255)
            )
            pipeline.osd_img.draw_string_advanced(
                8, 34, 18, "FPS=%.1f" % clock.fps(),
                color=(255, 255, 255)
            )
            pipeline.show_image()
